...
RegionTool::RegionTool(RasterImageEditor* editor, QUndoStack* undoStack)
    :mEditor(editor)
...
void RegionTool::OnDragBegin(int x, int y, float pressure)
{
    if (!mEditor->GetImage())
    {
        return;
...
void RegionTool::OnDrag(int x, int y, float pressure)
{
    if (!mEditor->GetImage())
    {
        return;
...
void RegionTool::OnDragEnd(int x, int y, float pressure)
{
    if (!mEditor->GetImage())
    {
        return;
...
void RegionTool::OnPaint(QPainter &p)
{
...
void RegionTool::BuildDrawSegment(const StrokePoint& p0, const StrokePoint& p1, QPainterPath& path)
{
    QPointF pts[4];
...
void RegionTool::DrawLastStroke()
{
    ...
    if (n >= 2)
    {
        ...
        {
            path.lineTo(samples[j].x, samples[j].y);
        }
...
void RegionTool::SetBrushSize(float value)
{
    if (value <= 0)
    {
        return;
...
void RegionTool::SetColor(const QColor& color)
{
    if (color == mColor)
    {
        return;
...
void RegionTool::SetSmooth(int value)
{
    if (value < 0)
    {
        value = 0;
...
void RegionTool::SetMode(QPainter::CompositionMode mode)
{
    mBrushMode = mode;
}
